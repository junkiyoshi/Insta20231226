...
void ofApp::draw() {
	...
	for (int j = 0; j < 2; j++) {
		...
		auto head_size = j == 0 ? 15 : 12;
		j == 0 ? ofSetColor(239, 39, 39) : ofSetColor(39);
		for (int k = 0; k < this->param_list.size(); k++) {
			...
			for (int i = 0; i < len; i++) {
				...
				auto noise_radius = ofMap(ofNoise(this->param_list[k].y, (ofGetFrameNum() + i) * 0.005), 0, 1, -360, 360);
				auto next_noise_deg = ofMap(ofNoise(this->param_list[k].x, (ofGetFrameNum() + i + 1) * 0.005), 0, 1, -360, 360);
				auto next_noise_radius = ofMap(ofNoise(this->param_list[k].y, (ofGetFrameNum() + i + 1) * 0.005), 0, 1, -360, 360);
